 ...
 * @param list any[]
 * */
export function addSpansValueInList(columnSpans: string[], list: any[]): any[] {
  calculateRowSpans(columnSpans, list);
  return list;
 ...
 * */
type CustomCellType = TableColumnType extends { customCell?: infer T } ? T : never;

export function setCustomCell(row_key: string, customCell?: CustomCellType): CustomCellType {
  return (record, rowIndex, column) => {
    const baseCell = typeof customCell === 'function' ? customCell(record, rowIndex, column) : {};
    return {
      ...baseCell,
      rowSpan: record[prefix + row_key],
    };
  };
}
